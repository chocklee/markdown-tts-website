...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (rows[0] && rows[0].emailVerified) {
      ...
      await client.query('DELETE FROM password_resets WHERE user_id = $1', [rows[0].id])
      await client.query('INSERT INTO password_resets (email, token, expires_at, user_id) VALUES ($1, $2, $3, $4)', [
        email,
        token,
        expiresAt,
        rows[0].id,
