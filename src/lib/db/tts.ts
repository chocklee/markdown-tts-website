...
export async function getCachedAudio(
  ...
  const { rows } = await pool.query<{ audio: Buffer; content_type: string }>(
    'SELECT audio, content_type FROM tts_cache WHERE provider = $1 AND voice = $2 AND text_hash = $3',
    [provider, voice, textHash],
  )
  const row = rows[0]
  if (!row) return null
...
export async function upsertCachedAudio(
  ...
  await pool.query(
    `INSERT INTO tts_cache (provider, voice, text_hash, audio, content_type, chars)
     VALUES ($1, $2, $3, $4, $5, $6)
     ON CONFLICT (provider, voice, text_hash)
     DO UPDATE SET
       audio = EXCLUDED.audio,
       content_type = EXCLUDED.content_type,
       chars = EXCLUDED.chars,
       created_at = now()`,
    [provider, voice, textHash, audio, contentType, chars],
  )
}
