...
beforeEach(async () => {
  ...
  for (const d of docs) await deleteDocument(d.docId)
...
describe('runSync', () => {
  ...
  it('成功上传脏文档并清除 dirty 标记', async () => {
    await putDocument(makeDoc('a', { dirty: true }))
    const result = await runSync()
  ...
  it('PUT 409 → 拉取服务端版本并计数冲突，本地内容被替换', async () => {
    await putDocument(makeDoc('a', { dirty: true, updatedAt: 100, content: 'local-content' }))
    const server = {
      docId: 'a',
      title: 't',
